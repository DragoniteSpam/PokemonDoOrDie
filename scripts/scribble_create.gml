/// list scribble_create(string, [minLineHeight], [maxLineWidth], [startingColour], [startingFont], [startingHAlign], [dataFieldsArray]);
...
if ( global.__scribble_init_complete == SCRIBBLE_INIT_START ) {
    show_error( "scribble_create() should be called after initialising Scribble.", false );
    exit;
}
...
if ( !ds_map_exists( global.__scribble_font_data, _def_font ) ) {
    show_error( "" + string( _def_font ) + " not recognised as a font", false );
    var _def_font = global.__scribble_default_font;
}
...
if ( _font_glyphs_array == undefined ) {
    ...
    if ( _array == undefined ) {
        show_error( "The space character is missing from font definition for " + _def_font, true );
        exit;
    }
}
...
if ( is_string( _def_colour ) ) {
    ...
    if ( _value == undefined ) {
        show_error( "The starting colour (" + _def_colour + ") has not been added as a custom colour. Defaulting to c_white.", false );
        _value = c_white;
    }
    _def_colour = _value;
}
...
if ( is_array( _data_fields_in ) ) {
    ...
    if ( _length > SCRIBBLE_MAX_DATA_FIELDS ) {
        show_error( "Length of custom data field array (" + string( _length ) + ") is greater than SCRIBBLE_MAX_DATA_FIELDS (" + string(SCRIBBLE_MAX_DATA_FIELDS) + ")", false )
...
// Break down string into sections using a buffer
...
repeat( _buffer_size ) {
    ...
    if (_in_command_tag) {
        ...
        if (_value == 10) || (_value == 32) || (_value == SCRIBBLE_COMMAND_TAG_OPEN /* or <space> or a command tag open character */ ) {
...
var _text_flags           = array_create( SCRIBBLE_MAX_FLAGS);
array_clear(_text_flags, 0);
...
for( var _i = 0; _i < _separator_count; _i++ ) {
    ...
    if ( _in_command_tag ) {
        ...
        if ( _sep_char != SCRIBBLE_COMMAND_TAG_CLOSE /* ] */ ) {
            ...
            switch( _parameters_list[| 0 ] ) {
                ...
                case "":
                    ...
                    _text_flags       = array_create( SCRIBBLE_MAX_FLAGS);
                    array_clear(_text_flags, 0);
                ...
                // Flags, fonts, sprites, and colours
                                ...
                                if ( _asset >= 0 ) && ( asset_get_type( _parameters_list[| 0] ) == asset_sprite ) {
                                    ...
                                    if ( _colour != undefined ) {
                                        ...
                                        if ( string_length( _colour_string ) <= 7 ) && ( string_copy( _colour_string, 1, 1 ) == "$" ) {
                                            ...
                                            var _lsf = ternary( _ord >= global.__scribble_hex_min && _ord <= global.__scribble_hex_max, global.__scribble_hex_array[ _ord - global.__scribble_hex_min ], 0);
                                            var _ord = ord( string_char_at( _colour_string, 2 ) );
                                            var _hsf = ternary( _ord >= global.__scribble_hex_min && _ord <= global.__scribble_hex_max, global.__scribble_hex_array[ _ord - global.__scribble_hex_min ], 0);
        ...
        // Find the substring's width and height
    ...
    if ( !_force_newline ) && ( _substr != "" ) {
        ...
        //Add a new word
        _new_word = true;
        var _word_array = array_create( __E_SCRIBBLE_WORD.__SIZE);
        array_clear(_word_array, 0);
...
// Set box width/height and adjust line positions based on alignment
...
repeat( _lines_size ) {
    ...
    repeat( _words_count ) {
        ...
        if ( _sprite != noone ) {
            ...
            // Check the font and texture to see if we need a new vertex buffer
            ...
            // Add vertex data for each character in the string
