...
if (target.mods[Stats.SPATTACK]<STAT_MOD_LIMIT){
    ...
    ds_queue_enqueue(queue, add_battle_individual_action(battle_individual_action_text, target.name+"'s Special Attack rose!"));
    ...
    ds_queue_enqueue(queue, add_battle_individual_action(battle_individual_action_text, target.name+"'s Special Attack won't go any higher!"));
}
