/// void scribble_add_event(name, script);
...
if ( !variable_global_exists( "__scribble_init_complete" ) ) {
    show_error( "scribble_add_event() should be called after initialising Scribble.", false );
    exit;
}

if ( !is_string( _name ) ) {
    show_error( "Event names should be strings.\n(Input to script was " + string(_name) + ")", false );
    exit;
}
...
if ( !script_exists( _script ) ) {
    show_error( "Script (" + string(_script) + ") doesn't exist!", false );
    exit;
}
