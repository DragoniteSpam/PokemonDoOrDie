        ...
        if (victorious==Camera.battle_pawn){
            ...
            if (reward>0){
                ...
                if (instanceof(victorious, PawnWild)){
                    var loss_message=L('%0 dropped %1 in panic!', Camera.battle_pawn.name, reward);
                } else {
                    var loss_message=L('%0 handed out %1 to the winner!', Camera.battle_pawn.name, reward);
                }
            ...
            ds_queue_enqueue(Battle.round_actions, add_battle_round_action(battle_round_action_text, L('%0 zonked out!', Camera.battle_pawn.name)));
            ds_queue_enqueue(Battle.round_actions, add_battle_round_action(battle_fade, c_black, fade_time, 0));
