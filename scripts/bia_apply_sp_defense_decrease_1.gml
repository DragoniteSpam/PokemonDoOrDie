...
if (target.mods[Stats.SPDEFENSE]>-STAT_MOD_LIMIT){
    ...
    ds_queue_enqueue(queue, add_battle_individual_action(battle_individual_action_text, target.name+"'s Special Defense fell!"));
    ...
    ds_queue_enqueue(queue, add_battle_individual_action(battle_individual_action_text, target.name+"'s Special Defense won't go any lower!"));
}
