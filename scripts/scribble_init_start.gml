/// void scribble_init_start(fontDirectory, texturePageSize)
...
if ( !directory_exists( _font_directory ) ) {
    show_error( "Font directory " + string( _font_directory ) + " could not be found in " + game_save_id + "!", true );
    exit;
}
...
if ( round(_exp) != _exp ) {
    ...
    show_error( "Texture page size " + string( _tpage_size ) + " isn't a power of 2. Did you mean " + string( _fixed_tpage_size ) + "? ", false );
    _tpage_size = _fixed_tpage_size;
}

if (_tpage_size < 512) || (_tpage_size > 8192) {
    show_error( "Texture page size " + string( _tpage_size ) + " is not supported by Scribble! ", true );
    exit;
}
...
global.__scribble_init_complete     = SCRIBBLE_INIT_DURING;
