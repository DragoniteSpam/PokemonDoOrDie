...
for (var i=0; i<ds_list_size(argument0); i++){
    if (argument0[| i]==-1){
        ...
        ds_list_add(text_list, get_move(argument0[| i]).name);
        if (this is set up to use a list of DataMove but should be set up to take a BattlePokemon instead thanks
    }
}
